...
def small_chat_delete(data_streams):
    ...
    data = delete_one_recplic_chat(data)
    #Push data into streams
...
def add_index(data_streams):
    assert (data_streams[8]['FUNCTION'] is small_chat_delete)
    assert (data_streams[10]['FUNCTION'] is sentence_concat)
    man_data = data_streams[8]['DATA']
    context_data = data_streams[10]['DATA']
    #Perfom function task
    print_out('add_index: list,list => list')
    data = add_indexing(context_data,man_data,2)
    #Push data into streams
...
# tiny_sber_data_csv = path.join(dataset_dir,'Dialog2_ разметка_чатов_MK_v01.csv')
...
assert (data_streams[11]['FUNCTION'] is add_index)
assert (data_streams[10]['FUNCTION'] is sentence_concat)
full_chats = zipper(data_streams[10]['DATA'], data_streams[11]['DATA'])
